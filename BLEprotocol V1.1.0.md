...
| **0x50** | 查询工作日志   | 查询工作日志                    | √ | * |
| 0xC0     | 回馈日志         |                                                    | √ | * |
| **0x51** | 工作日志查询成功 | 工作日志查询成功，并进行下一次通讯        | √ | * |
|          |                  |                                                    |          |          |
| **0x52** | 清空锁工作日志   | 工作日志查询成功，并结束通讯                | √ | * |
| 0xC2     | 操作成功 |                                                    | √ | * |
...
- 通过刷手环方式添加钥匙，发送完此指令后1分钟内刷手环会自动添加，
...
- 校准传感器，该指令有两个模式，在校准无物体模式下，清空所有工具，发送校准无物体指令，此时硬件记录传感器数值，然后再将所有工具摆放到位，发送校准有物体指令。

- 此指令在第一次使用内衬时，或者工具监测不准确时使用.
...
- 回馈指令用于回馈删除结果
  ...
  **total：**表示抽屉总数量（还没更新固件）。

  **val：** 表示抽屉属性，total的值是N，这里的数据就有（N*2）字节。Val[0]表示底第几层抽屉（也就是抽屉板的硬件地址），Val[1]表示抽屉型号，对应抽屉型号表格对应。

例如：payload数据为：{0x00,0x04,0x01,0x04,0x02,0x0E}；B2表示该工具箱有2个抽屉，B3-4第一个抽屉的硬件地址是0x01，类型代码为0x04，内衬型号为F1ET04；B5-6表示第二个抽屉的硬件地址是0x02，类型代码为0x0E，内衬型号为F1ET14。
...
例如：payload数据为：{0x01,0xFF,0xFF}；B1表示当前抽屉已经打开，11个传感器均有遮挡。（不同型号内衬锁对应的Val值的字节数不同）
...
* 0x11:抽屉成功放回日志，记录抽屉成功放回时的日志，该数据分2个包进行传输，
* 包1传输数据如下
...
| 抽屉编号 | 抽屉类型 | 抽屉状态 | 抽屉传感器状态 |
| -------- | -------- | -------- | -------------- |
| 1Byte    | 1Byte    | 1Byte    | 4Byte（暂定）  |



传送完成后，APP应主动下发成功日志指令。
